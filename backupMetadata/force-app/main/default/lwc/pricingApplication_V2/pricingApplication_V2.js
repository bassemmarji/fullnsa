...
import NSA_ApplicationVersion from '@salesforce/label/c.NSA_ApplicationVersion';
...
import providerLine5 from '@salesforce/label/c.NSA_ProviderLine5';
import providerLine6 from '@salesforce/label/c.NSA_ProviderLine6';
...
//Backend Functions
import getProcedureCodesByHPI from "@salesforce/apex/EOBEntryFormController_V2.getProcedureCodesByHPI";
import getHealthPlanItems from "@salesforce/apex/EOBEntryFormController_V2.getHealthPlanItems";
import checkEmployerGroup from "@salesforce/apex/EOBEntryFormController_V2.checkEmployerGroup";
//import getProviderByNPI from "@salesforce/apex/EOBEntryFormController_V2.getProviderByNPI";
import createJiraTicket from "@salesforce/apex/EOBEntryFormController_V2.createJiraTicket";
import hasOutOfNetwork from "@salesforce/apex/EOBEntryFormController_V2.hasOutOfNetwork";
import handleSQLError from "@salesforce/apex/EOBEntryFormController_V2.handleSQLError";
import getNetworksDetails from "@salesforce/apex/EOBEntryFormController_V2.getNetworksDetails";
import getTPAAcro from "@salesforce/apex/EOBEntryFormController_V2.getTPAAcro";
import insertSL from "@salesforce/apex/EOBEntryFormController_V2.serviceLineInsert";
import serviceLineInsertPodPlan from "@salesforce/apex/EOBEntryFormController_V2.serviceLineInsertPodPlan";
import getPatientInfo from "@salesforce/apex/EOBEntryFormController_V2.getPatientInformation";
import getPatient from "@salesforce/apex/EOBEntryFormController_V2.getPatient";
import getHPIList from "@salesforce/apex/EOBEntryFormController_V2.getHPIList";
import getHPCList from "@salesforce/apex/EOBEntryFormController_V2.getHPCList";
import checkExistingNPI from "@salesforce/apex/EOBEntryFormController_V2.checkExistingNPI";
//import getPatientDependents from "@salesforce/apex/EOBEntryFormController_V2.getPatientDependents";
import triggerCalculation from "@salesforce/apex/EOBEntryFormController_V2.triggerCalculation";
import getProcedureCodes from "@salesforce/apex/EOBEntryFormController_V2.getProcedureCodes";
import downloadPDF from '@salesforce/apex/EOBEntryFormController_V2.getPdfFileAsBase64String';
import serversHeartbeat from "@salesforce/apex/EOBEntryFormController_V2.serversHeartbeat";
...
export default class PricingApplication extends LightningElement {
    @track nonsimilarNetworkOptionProviders=false;
    @api logoUrl;
    @api fieldSetName;
    ...
    @api isProvider;
    @api isPodPlan;
    @track questionsDateError=false;
    @track anyvalidationErrorExists=true;
    @track questionsDateChangeError;
    @track selectedPatientName;
    @track isEligibilityLoaded=true;
    @track eligibilitySelected=[];
    @track eligibilityList=[];
    @track patientNotSelectedError;
    @track patientSelectedAfterEligLoad=false;
    @track eligibleFrom;
    @track mainPlanMember=[];
    @track listOfPatientIds=[];
    @track customerPortal=false;
    @track eligibleUntil;
    @track listOfHPIsDisplay=[];
    @track NSA_ApplicationVersion=NSA_ApplicationVersion;
    @track isCPTLoaded=true;
    @track healthPlanIDlabel;
    @track searchResults;
    @track selectedSearchResult;
    @track newVersion=false;
    @track healthPlanItemId;
    @track listOfHPIs=[];
    @track hpiselected;
    @track searchByCPTOption=false;
    @track searchBySpecialityOption=false;
    ...
    @track providerLine5 = providerLine5;
    @track providerLine6 = providerLine6;
    ...
    selectedProceduresConfirmationcolumnsResults = [
        {
            ...
            fieldName: 'IntPS__L2110_Service_Date__c',
            type: 'datePicker',
            editable:false,
            headerSort: false,
            hideDefaultActions: true,
            ...
            typeAttributes: {
                placeholder: 'Choose Your Service Date',
                value: '' // default value for picklist
                    ,
                context: {
                    fieldName: 'Name'
                },
    ...
    @track SelectedNPIcolumnsResults = [
        {
            label: 'Action',
            type: 'button',
            hideDefaultActions: true,
            ...
            typeAttributes: {
                ...
                variant: 'bare',
                alternativeText: 'Remove Provider',
                label: 'Remove',
    ...
    @track NPIcolumnsResults = [
        {
            ...
            typeAttributes: {
                ...
                variant: 'bare',
                alternativeText: 'Choose Provider',
                label: 'Add',
                ...
                checkEmployerGroup({
                 ...
                 this.isEmployerActivated=result;
                    ...
                    result.forEach(dependent => {
                        ...
                        console.log('dependen2t ' + dependent.IntPS__Plan_Member__c)
                        if(dependent.IntPS__Plan_Member__c==null){
                            console.log('filling main member');
                            const element = {
                                label: dependent.Name,
                                value: dependent.Id
                            };
                            this.mainPlanMember=[...this.mainPlanMember,  element];
                            console.log('main member '+JSON.stringify(this.mainPlanMember));

                        }
                        else{
                            console.log('filling dependent member');
                            const element = {
                                label: dependent.Name,
                                value: dependent.Id
                            };
                        ...
                        this.listOfPatientIds = [...this.listOfPatientIds, dependent.Id];
                        console.log('dependents ' + JSON.stringify(this.dependents));
                    })
                
                    this.getPatientInformation(this.todaysDate);
    ...
    //handles
    //handle when a user selects a plan member to do the advanced eob on
    handleNameSelection(event){
        console.log('inside handle name selection');
        console.log('You selected the patient: ' + event.target.valeue);
        this.eligibilitySelected=[];
        ...
        this.listOfPatientIds = [];
        this.providerSelected = false;
        this.providerId = null;
        this.procedureCodeList=[];
        if(this.template.querySelector('c-nsa_provider-search')!=null){
            this.template.querySelector('c-nsa_provider-search').refreshAll();

        }

        this.networkOption = '';
        this.isEligibilityLoaded=false;
        this.eligibilityId = ''
        this.clientId = '';
        this.clientName = '';
        this.healthPlanName='';
        ...
        this.listOfPatientIds = [...this.listOfPatientIds, this.dependentValue];
        console.log('dependentValue' +this.dependentValue);
        this.patientSelected=true;
        this.openEOBInfoSection = true;
        let selectedId = event.target.id;

        // Retrieve the corresponding label using the 'for' attribute of the label element
        let labelElement = this.template.querySelector(`label[for="${selectedId}"]`);
    
        // Retrieve the label text
        this.selectedPatientName = labelElement.textContent;
       // this.showToast('Patient Selected', 'Patient Selected!', 'You have selected: '+this.selectedPatientName);
        console.log(JSON.stringify(this.eligibilityList));

        for (let i = 0; i < this.eligibilityList.length; i++) {
            console.log(this.eligibilityList[i].IntPS__Patient__c+' compare '+this.patientId);
            if(this.eligibilityList[i].IntPS__Patient__c===this.patientId){
                console.log()
                this.eligibilitySelected = [...this.eligibilitySelected, this.eligibilityList[i]];
                break;
            }
        }

       
        console.log('elig list filtered');
        console.log(JSON.stringify(this.eligibilitySelected));
        if(this.eligibilitySelected.length>0){
        this.eligibilityId = this.eligibilitySelected[0].Id;
        this.clientId = this.eligibilitySelected[0].IntPS__Patient__r.IntPS__Employer__c;
        this.clientName = this.eligibilitySelected[0].IntPS__Patient__r.IntPS__Employer__r.Name;
        this.healthPlanName=this.eligibilitySelected[0].IntPS__Health_Plan__r.Name;
        this.eligibleFrom=this.eligibilitySelected[0].IntPS__Begin_Date__c;
        this.eligibleUntil = this.eligibilitySelected[0].IntPS__Term_Date__c ?? 'N/A';
        this.accId = this.eligibilitySelected[0].IntPS__Patient__r.IntPS__Employer__c;
        this.healthPlanId = this.eligibilitySelected[0].IntPS__Health_Plan__c;
        ...
        this.getHealthPlanItemsFromBackend();
      //  this.showToast('success', 'Success!', this.NSA_ElligibilityFound);
        ...
        this.patientSelectedAfterEligLoad=true;
        this.patientNotSelectedError='';
        this.isEligibilityLoaded=true;
        this.openEOBInfoSection=true;
    }
    else
    {
        ...
        this.openEOBInfoSection=true;
        this.isEligibilityLoaded=true;
        this.patientSelectedAfterEligLoad=false;
        this.patientNotSelectedError='The Selected Patient Has No Active Eligibility.';  
    }
    }
    ...
    handlePatientLookupChange(event) {
     ...
     //when a user selects a health plan item, we search for all cpt codes in this choice
     handleHPIChange(){
        this.getProcedureCodesByHPIfromBackend();
    }
    ...
    handleDateCellChange(event) {
        this.questionsDateChangeError='';
        this.questionsDateError=false;
        ...
        console.log('test handle');
            ...
            var today = new Date(event.detail.data.value);
            var dd = String(today.getDate()).padStart(2, '0');
            var mm = String(today.getMonth() + 1).padStart(2, '0'); //January is 0!
            var yyyy = today.getFullYear();
            this.searchedDate=yyyy + '-' + mm + '-' + dd;
            console.log('NEW Search date '+this.searchedDate);

       
            
            
            selectedProceduresMock[index].IntPS__L2110_Service_Date__c = this.searchedDate;
            console.log('setting enw date '+selectedProceduresMock[index].IntPS__L2110_Service_Date__c)
        
        for (let i = 0; i < selectedProceduresMock.length; i++) {
               console.log('checking all cpt codes chosen '+selectedProceduresMock[i].IntPS__L2110_Service_Date__c);
            if(this.eligibleUntil=='N/A'){
                if(selectedProceduresMock[i].IntPS__L2110_Service_Date__c<this.eligibleFrom || selectedProceduresMock[i].IntPS__L2110_Service_Date__c==null || selectedProceduresMock[i].IntPS__L2110_Service_Date__c==''){
                    this.questionsDateChangeError='Dates Of Service should be after: '+this.eligibleFrom;
                    this.questionsDateError=true;
                    break;
                }
            }
            else{
                if(selectedProceduresMock[i].IntPS__L2110_Service_Date__c<this.eligibleFrom || selectedProceduresMock[i].IntPS__L2110_Service_Date__c>this.eligibleUntil || selectedProceduresMock[i].IntPS__L2110_Service_Date__c==null || selectedProceduresMock[i].IntPS__L2110_Service_Date__c==''){
                    this.questionsDateChangeError='Dates Of Service should be between: '+this.eligibleFrom+' And : '+this.eligibleUntil;
                    this.questionsDateError=true;
                    break;
                }
            }
        ...
        if(!this.questionsError && !this.questionsDateError){
            this.anyvalidationErrorExists=false;
        }
        else{
            this.anyvalidationErrorExists=true
        }
    }
    ...
    searchHPIs(event) {
        ...
        const result = this.listOfHPIsDisplay.filter((picklistOption) =>
      ...
      selectSearchResult(event) {
        ...
        this.healthPlanItemId = event.currentTarget.dataset.value;
        this.selectedSearchResult = this.listOfHPIsDisplay.find(
            (picklistOption) => picklistOption.value === this.healthPlanItemId
        );
        this.healthPlanIDlabel=this.selectedSearchResult ? this.selectedSearchResult.label : null;
        console.log(this.healthPlanIDlabel);
        this.clearSearchResults();
        this.handleHPIChange();
      }
      showPicklistOptions() {
        if (!this.searchResults) {
            this.searchResults = this.listOfHPIsDisplay;
        }
      }
    handleNPIDeleteRowAction(event) {
        console.log('Testing removing');
        
        const selectedProviderMock = [...this.selectedProvider];
        const deletedNPINumber = event.detail.row.NPINumber;
    
        // Use Array.prototype.findIndex for a more concise way to find the index
        const indexToDelete = selectedProviderMock.findIndex(provider => provider.NPINumber === deletedNPINumber);
    
        if (indexToDelete !== -1) {
            selectedProviderMock.splice(indexToDelete, 1);
        }
    
        this.selectedProvider = selectedProviderMock;
        this.NPIList = [...this.NPIList, event.detail.row];
    }
    
    handleNPIRowAction(event) {
        console.log('Testing adding');
        this.nonsimilarNetworkOptionProviders=false;
        const selectedProvider = event.detail.data.row;
        console.log('Selected Provider:1'+JSON.stringify(selectedProvider))
        console.log('Selected Provider:1 '+this.networkOption);
        console.log('Selected Provider:1 '+this.selectedProvider.length);
        console.log('Selected Provider:1 '+selectedProvider.networkOption);

        if( this.selectedProvider.length==0 || this.networkOption==null || this.networkOption==selectedProvider.networkOption){
            console.log('Selected Provider:1');
            this.selectedProvider = [...this.selectedProvider, selectedProvider];
            console.log('Selected Provider:2');
        this.providerSelected = true;
        console.log('Selected Provider:3');
        this.providerId = selectedProvider.ProviderName;
    
        console.log('Selected Provider:', this.selectedProvider);
    
        this.networkOption = selectedProvider.networkOption;
        console.log('Network Option:', this.networkOption);
    
        this.submitacc = true;
        this.searchForProcedures = true;
    
        const searchedProviderMock = [...this.NPIList];
        const addedNPINumber = selectedProvider.NPINumber;
    
        // Use Array.prototype.findIndex for a more concise way to find the index
        const indexToDelete = searchedProviderMock.findIndex(provider => provider.NPINumber === addedNPINumber);
    
        if (indexToDelete !== -1) {
            searchedProviderMock.splice(indexToDelete, 1);
        }
    
        this.NPIList = searchedProviderMock;
    }
    else{
        this.nonsimilarNetworkOptionProviders=true;
    }
    }
    ...
    handleRowSelected(event) {
        ...
        this.selectedProvider = [...this.selectedProvider, selectedRows[0]];
        const row = (JSON.stringify(this.selectedProvider));

        console.log('selectedProvider '+row);

        
        this.networkOption = this.selectedProvider[0].networkOption;
        this.listOfHPIsDisplay = this.listOfHPIs.filter((user) => {
            return  user.networkoption === this.networkOption;
        });
        console.log((JSON.stringify(this.listOfHPIsDisplay)));
    ...
    handleRowAction(event) {
        ...
        var row;
        if(this.newVersion){
             row = JSON.parse(JSON.stringify(event.detail.data.row));

        }
        else{
             row = JSON.parse(JSON.stringify(event.detail.row));
        } 
        console.log('client2222 ' + this.clientName);
        this.searchedDate=this.todaysDate;
        ...
        this.selectedProcedures = this.selectedProcedures.map((acc) => ({ ...acc, dateDisabled: true}));
    ...
    handleEobFormSubmit(event) {
        ...
        this.selectedProcedures = this.selectedProcedures.map((acc) => ({ ...acc, dateDisabled: false}));
        ...
        if (this.isPodPlan) {
            serviceLineInsertPodPlan({
                ...
                providerNumber: this.selectedProvider[0].NPINumber,
                ...
                providerZIP: this.selectedProvider[0].ProviderPostalCode,
            ...
            console.log('outOfNetworkNames test '+row);
            insertSL({
                claimFields: this.submittedEob,
                procedures: JSON.stringify(this.proceduresToInsert),
                selectedproviderList: JSON.stringify(this.selectedProvider),
                serviceDate: this.searchedDate,
                networks: this.networks,
                networkOption: this.networkOption,
                tpaAcro: this.tpaAcro,
                ...
                availableNetworksForProvider:this.availableNetworksForProvider,
                applicationVersion:this.NSA_ApplicationVersion
            }).then((result) => {
                this.eobSubmitted = true;
                this.eobError = false;
                this.eobUrlOK = true;
                ...
                this.SelectedNPIcolumnsResults = this.SelectedNPIcolumnsResults.filter(col => col.label != 'Remove');
                this.NPIcolumnsResults = this.NPIcolumnsResults.filter(col => col.label != 'Choose');
    ...
    confirmSelectedProceduresConfirmation(event){
        const isConfirmed = window.confirm('Once Confirmed, you will not be able to change the selected Provider or the selected services.Are you sure you want to proceed?');
        if (isConfirmed) {
            // User confirmed, proceed with the action
            this.confirmSelectedProcedures(event);
        }
    }
    //used when the user presses on the button Confirm Selected Procedures
      ...
      confirmSelectedProcedures(event) {
        ...
        this.template.querySelector('c-nsa_provider-search').disableAllInputs();
    ...
    checkQuestionsAsked(){
        for (let i = 0; i < this.selectedProcedures.length; i++) {
            console.log('this.selectedProcedures[i] '+JSON.stringify(this.selectedProcedures[i]));
            ...
            if(this.selectedProcedures[i].IntPS__L2110_Service_Date__c=='' || this.selectedProcedures[i].IntPS__L2110_Service_Date__c==null ){
                if(this.eligibleUntil=='N/A'){
                    this.questionsDateChangeError='Dates Of Service should be after: '+this.eligibleFrom;
                    this.questionsDateError=true;
                }
...
if(!this.questionsError && !this.questionsDateError){
  ...
  /*  handleVFResponse(message) {
    ...
    connectedCallback() {
        if(NSA_ApplicationVersion=='2.0'){this.newVersion=true;}
        serversHeartbeat()
        .then(data => {
        ...
        this.searchValue = this.todaysDate;
    ...
    createProcedureListObject() {
        for (let i = 0; i < this.selectedProcedures.length; i++) {
            const element = {
                serviceDate: this.selectedProcedures[i].IntPS__L2110_Service_Date__c,
    ...
    //when choosing date, we check if the date picked is older than today to make the user select future date
     handleServiceDateChange(event){
        ...
        console.log('event.detail.data.value ' + event.detail.data.value);
        console.log(event.detail.data.value+ ' '+this.todaysDate);
        if(new Date(event.detail.data.value) < new Date(this.todaysDate)){
        
            if(event.detail.data.value.toString()==this.todaysDate.toString()){
                this.oldDate=true;
            }else{
                this.oldDate=false;
            }
     ...
     getPatientInformation(tempServiceDate){
        ...
        getPatientInfo({
            patientId: this.listOfPatientIds,
            serviceDate: tempServiceDate,
            isCustomerPortal: this.customerPortal
         }).then((result) => {
            ...
            if (result.Eligibility != null && result.Eligibility.length > 0) {
                if (result.Eligibility.length == 1) {
                    this.eligibilityList=result.Eligibility;
                    ...
                    this.eligibleFrom=result.Eligibility[0].IntPS__Begin_Date__c;
                    this.eligibleUntil = result.Eligibility[0].IntPS__Term_Date__c ?? 'N/A';
                    console.log('22');
                   ...
                   // this.showToast('success', 'Success!', this.NSA_ElligibilityFound);
                    ...
                    this.patientSelectedAfterEligLoad=false;
                    this.patientNotSelectedError='You Must Select A Patient first In Order To View the Eligibility Details';
                    this.eligibilityList=result.Eligibility;
    ...
    //get all hpis of a health plan
    getHealthPlanItemsFromBackend(){
        console.log('getting health plan items of '+this.healthPlanId);
        getHealthPlanItems({
            healthplan: this.healthPlanId       
        }).then((result) => {
            console.log('this.listOfHPIs.length ' + result.length);
            console.log((JSON.stringify(result)));
            for (let i = 0; i < result.length; i++) {
                console.log('filling in hpis names');
                this.listOfHPIs.push({label: result[i].Name, value: result[i].Id,networkoption:result[i].IntPS__Health_Plan_Content__r.IntPS__Provider_Network__c});
            }
            console.log((JSON.stringify(this.listOfHPIs)));



        }).catch((error) => {
            console.log('error in hpi' + error);
        });
    }
    //get all cpt codes of a health plan item
    getProcedureCodesByHPIfromBackend(){
        this.isCPTLoaded=false;
        console.log(' this.isCPTLoaded '+ this.isCPTLoaded);
        getProcedureCodesByHPI({
            hpi: this.healthPlanItemId       
        }).then((data) => {
            this.isProceduresLoaded = true;
            this.isCPTLoaded=true;

            if (data != null && data.length > 0) {
                data.forEach(ele => {
                    console.log('ele ' + ele.Status);
                    console.log('ele3 ' + ele.buttonsDisable);
                    ele.format = ele.Status == 'This Service is not Covered in your Health Plan. You will have to pay its full price' ? 'slds-text-color_error' : 'slds-text-color_success';
    ...
    searchByCPTOptionChange(event){
        this.procedureCodeList=[];
        this.removeEligibilityDiv();
        this.removeNPIDiv();
        this.activateProceduresDiv();
        // this.clickedButtonLabel = event.target.label;
        this.searchBySpecialityOption=false;
        this.searchByCPTOption = event.detail.checked;
        if (event.detail.checked) {
            Array.from(this.template.querySelectorAll('lightning-input'))
                .forEach(element => {
                    if (element.name == "specialitysearch") {
                        element.checked = false;
                    }
    ...
    searchBySpecialityOptionChange(event){
        this.procedureCodeList=[];
        this.removeEligibilityDiv();
        this.removeNPIDiv();
        this.activateProceduresDiv();
        // this.clickedButtonLabel = event.target.label;
        this.searchByCPTOption=false;
        this.searchBySpecialityOption = event.detail.checked;
        if (event.detail.checked) {
            Array.from(this.template.querySelectorAll('lightning-input'))
                .forEach(element => {
                    if (element.name == "cptsearch") {
                        element.checked = false;
                    }
